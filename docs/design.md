...
### R4. 스케줄 조사는 요일의 생성 시각(open_time)에 tick이 생성, 마감이 지났으면 건너뜀

```
tick(1분) 또는 홈 접속 ──▶ 오늘 요일과 일치하는 enabled 스케줄마다
          ├─ 아직 open_time 전이다 ──▶ 건너뜀 (다음 tick에 다시)
          ├─ 오늘 마감시각이 이미 지났다 ──▶ 건너뜀 (stale skip)
          └─ 아니면 INSERT OR IGNORE surveys (schedule_id, survey_date=오늘) → 생성됐으면 텔레그램 알림
```

- `open_time`(기본 00:00, 폼 기본 08:00)이 지난 첫 tick이 만든다. 생성 알림이 그 시각에 가므로 새벽에 울리지 않게 조정할 수 있다.
  아무도 응답하지 않아도 마감 때 자동 채택된 주문서가 나온다 — 그게 의도다.
...
### R8. 텔레그램 알림은 "가장 가까운 한 채팅", 세 시점

- 시점: **생성**(수동·스케줄), **마감 30분 전·10분 전 리마인더**(미응답자가 있을 때만, 이름 포함, "약 N분 후"는 실제 남은 시간), **마감**(주문서 텍스트).
- 리마인더는 **기본 꺼짐.** 시점은 **조사 → 그룹(가장 가까운 상위 순)** 순으로 처음 값이 있는 곳을 쓰고, 어디에도 없으면 보내지 않는다.
  각 단계 값은 `'30,10'`(지정) / `'0'`(끔) / NULL(상속). 스케줄에 적은 값은 생성되는 조사에 복사된다. 전역 기본값은 두지 않았다 —
  "설정 안 했는데 알림이 온다"보다 "설정하면 온다"가 예측 가능하다.
- 수동 발송: 조사 상세의 "지금 알림 보내기"(생성자·관리자). 미응답자 목록을 즉시 보낸다. **조사당 3분에 1회**(`notified_at`) — 버튼을 연타해 채팅을 도배하지 못하게.
  자동 리마인더 표식(`reminded_at`)과는 별개라 서로 영향이 없다.
- 각 자동 시점마다 한 번: `reminded_at`(마지막 전송 시각)이 그 시점보다 앞이면 아직 안 보낸 것. 서버가 꺼져 있다
  켜져 두 시점이 다 지났으면 가장 임박한 것 하나만. 각 시점 이전에 존재하던 조사만 대상 — 급히 만든 조사에 연달아 울리지 않게.
- 그룹마다 `telegram_chat_id`(선택). `notify_targets(group)`이 정한 채팅으로 보낸다.
...
결제자 뽑기 · 정산 기록 · 복수 카페 후보 투표 · 메뉴 텍스트 붙여넣기 파서 · HTMX 부분 갱신
