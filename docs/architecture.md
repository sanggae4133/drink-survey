...
disabled  → 모든 요청 거절(기존 쿠키 포함). 조사 대상·자동 채택에서도 제외
          → 본인이 만든 조사 마감·삭제 / 본인이 만든 스케줄 on·off·삭제 / 본인이 추가한 게스트 잔 삭제
admin     → 위 전부 + 회원 사전 등록·수정, 그룹 트리·멤버 관리, 모든 조사 마감·삭제, 모든 스케줄 삭제, 모든 게스트 잔 삭제, 메뉴 가져오기, 카페 삭제
```
...
| 파일 가져오기 | 관리자만, 64KB, pydantic으로 형식·개수·범위 전체 검증, 갱신·추가만(삭제 없음), 파일명·MIME 불신 |
| 브라우저 캐시 | 모든 응답 `Cache-Control: no-store` — 뒤로가기 옛 화면·주문 내역 캐시 잔존 방지 |
...
- ORM, 마이그레이션 프레임워크 → 테이블 9개. `schema.sql`이 정답이고 `migrate.py`가 기존 DB를 그 모양으로 재구성한다(버전 파일 없음)
